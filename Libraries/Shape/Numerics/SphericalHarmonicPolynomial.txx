...
template< unsigned int TDimension >
double SphericalHarmonicPolynomial< TDimension >::ComputeGaussianCurvature (unsigned int from_l, unsigned int to_l, double phi, double theta) 
{
  double kappa[2] ;
...
template< unsigned int TDimension >
double SphericalHarmonicPolynomial< TDimension >::ComputeMeanCurvature (unsigned int from_l, unsigned int to_l, double phi, double theta) 
{
  double kappa[2] ;
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::ComputePrincipalCurveUV (unsigned int from_l, unsigned int to_l, double startPhi, double startTheta, double *curve, double stepSize, int curveLength) 
{
  int i, curveIndex ;
  double phi, theta ;
  double dir[4], prevDir[2] ;
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::GetOneRidgePoint (unsigned int from_l, unsigned int to_l, unsigned int family, double *uv, double *point) 
{
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::GradKappa (unsigned int from_l, unsigned int to_l, unsigned int family, double *point, double *gradKappa) 
{
	...
	TopDTheta = eDTheta + 2*lambda*fDTheta + 2*second[1]*lambdaDTheta +lambda*lambda*gDTheta + 2*second[2]*lambda*lambdaDTheta ;
	TopDPhi = eDPhi + 2*lambda*fDPhi + 2*second[1]*lambdaDPhi +lambda*lambda*gDPhi + 2*second[2]*lambda*lambdaDPhi ;
	BottomDTheta = EDTheta + 2*lambda*FDTheta + 2*first[1]*lambdaDTheta +lambda*lambda*GDTheta + 2*first[2]*lambda*lambdaDTheta ;
	BottomDPhi = EDPhi + 2*lambda*FDPhi + 2*first[1]*lambdaDPhi +lambda*lambda*GDPhi + 2*first[2]*lambda*lambdaDPhi ;
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::GetGradKappa (unsigned int from_l, unsigned int to_l, unsigned int family, double phi, double theta, double *dir) 
{
  double uv[2], point[2] ;
  point[0] = phi ;
  point[1] = theta ;
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::FollowRidge (unsigned int from_l, unsigned int to_l, unsigned int family, double *startPoint, double *uv, double *point, double stepSize) 
{
	double GradKappa[2] ;
...
template< unsigned int TDimension >
void SphericalHarmonicPolynomial< TDimension >::GetCaustic (unsigned int from_l, unsigned int to_l, int family, double phi, double theta, double *point) 
{
