...
{ 
  if (GetNumberOfOutputs() < 1) return 0;
#ifdef _MSC_VER
...
{ 
   if (GetNumberOfOutputs() < idx) return 0;
#ifdef _MSC_VER
...
double ParametricMeshToSPHARMSpatialObjectFilter::FlipAxesDistance(const CoefListType *coef1,const CoefListType * coef2)
{
 double diff[3];
 double sum = 0;
...
void ParametricMeshToSPHARMSpatialObjectFilter::EllipseScaleAlign(const CoefListType * incoef, CoefListType * outcoef)
{
  ...
  double a = sqrt(((*incoef)[1])[0] * ((*incoef)[1])[0] + ((*incoef)[1])[1] * ((*incoef)[1])[1] + ((*incoef)[1])[2] * ((*incoef)[1])[2]);
  double b = sqrt(((*incoef)[2])[0] * ((*incoef)[2])[0] + ((*incoef)[2])[1] * ((*incoef)[2])[1] + ((*incoef)[2])[2] * ((*incoef)[2])[2]);
  double c = sqrt(((*incoef)[3])[0] * ((*incoef)[3])[0] + ((*incoef)[3])[1] * ((*incoef)[3])[1] + ((*incoef)[3])[2] * ((*incoef)[3])[2]);
...
void ParametricMeshToSPHARMSpatialObjectFilter::
EllipseAlign(const CoefListType * incoef, CoefListType * outcoef, double scale)
{
  outcoef->clear();
  ...
  double a = sqrt(((*outcoef)[1])[0] * ((*outcoef)[1])[0] + ((*outcoef)[1])[1] * ((*outcoef)[1])[1] + ((*outcoef)[1])[2] * ((*outcoef)[1])[2]);
  double b = sqrt(((*outcoef)[2])[0] * ((*outcoef)[2])[0] + ((*outcoef)[2])[1] * ((*outcoef)[2])[1] + ((*outcoef)[2])[2] * ((*outcoef)[2])[2]);
  double c = sqrt(((*outcoef)[3])[0] * ((*outcoef)[3])[0] + ((*outcoef)[3])[1] * ((*outcoef)[3])[1] + ((*outcoef)[3])[2] * ((*outcoef)[3])[2]);
  // rotation axes
...
void ParametricMeshToSPHARMSpatialObjectFilter::
MakeEllipsoidAxisRightHand(CoefListType * coef)
{
  double a = sqrt(((*coef)[1])[0] * ((*coef)[1])[0] + ((*coef)[1])[1] * ((*coef)[1])[1] + ((*coef)[1])[2] * ((*coef)[1])[2]);
  double b = sqrt(((*coef)[2])[0] * ((*coef)[2])[0] + ((*coef)[2])[1] * ((*coef)[2])[1] + ((*coef)[2])[2] * ((*coef)[2])[2]);
  double c = sqrt(((*coef)[3])[0] * ((*coef)[3])[0] + ((*coef)[3])[1] * ((*coef)[3])[1] + ((*coef)[3])[2] * ((*coef)[3])[2]);
...
void ParametricMeshToSPHARMSpatialObjectFilter::
BestFlip (CoefListType * coef, const CoefListType * flipTemplate)
{
...
double ParametricMeshToSPHARMSpatialObjectFilterLegendre::P(int l, int m, float z) {
  ...
  if (m==0) {
    ...
    f= ((plm[l])[m])[l-m];
    for (i= l-m-1; i>=0; i--) f= z*f + ((plm[l])[m])[i]; 
    return f;
  }
}
...
void ParametricMeshToSPHARMSpatialObjectFilterLegendre::Ylm(int l, int m, float x, float y, float z,
  ...
  if (m < 0) {
    ylm(l, -m, x, y, z, re, im);
    if (m%2 == 0) im= -im; else re= -re;
  }
  else ylm(l, m, x, y, z, re, im);
  //cout << x << "," << y << "," << z << ": ";
  //cout << re << " " << im << ": ";
}
...
int 
ParametricMeshToSPHARMSpatialObjectFilter::Get_BaseVal( PointsContainerPointer paraPoints, float* &A, int &nvert, int &degree) 
{
...
{  
  ...
  if (surfPoints->Size() != paraPoints->Size()) {
    throw ParametricMeshToSPHARMSpatialObjectFilterException(__FILE__, __LINE__, "ParametricMeshToSPHARMSpatialObjectFilter: surface and para do not have same number of points");
  }
  ...
  delete A;
  A = NULL;
  if (m_flatCoeffs) delete m_flatCoeffs;
...
{  
  ...
  if (this->GetNumberOfInputs() < 2)
    {
    throw ParametricMeshToSPHARMSpatialObjectFilterException(__FILE__, __LINE__, "ParametricMeshToSPHARMSpatialObjectFilter: 2 surfaces are needed, a surface and a parameterization mesh");
    }
