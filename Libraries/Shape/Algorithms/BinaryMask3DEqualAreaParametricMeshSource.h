...
public:
  ...
  /** Constructor. */
 BinaryMask3DEqualAreaParametricMeshSourceException(const char *file, unsigned int line, 
                           const char* message = "Error in generating parametrized mesh from Binary Mask") : 
  ...
  /** Constructor. */
  BinaryMask3DEqualAreaParametricMeshSourceException(const std::string &file, unsigned int line, 
                           const char* message = "Error in generating parametrized mesh from Binary Mask") : 
    ExceptionObject(file, line)
  {
   SetDescription(message);
...
typedef DefaultDynamicMeshTraits< float , EqualAreaParametricMeshSourceMeshTypeDimension, EqualAreaParametricMeshSourceMeshTypeDimension, double > EqualAreaParametricMeshSourceMeshTrait;
typedef Mesh<float,EqualAreaParametricMeshSourceMeshTypeDimension,EqualAreaParametricMeshSourceMeshTrait> BinaryMask3DEqualAreaParametricMeshSourceMeshType;
...
public:
  ...
  virtual OutputMeshType * GetInitParametricMesh() { return m_InitParametricMesh.GetPointer();}
  virtual void SetInitParametricMesh ( OutputMeshType * in) { m_InitParametricMesh = in; m_InitParametricMeshSet = true;};
  virtual void ClearInitParametricMesh() { m_InitParametricMesh = NULL; m_InitParametricMeshSet = false;};
