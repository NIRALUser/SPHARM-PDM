...
int main(const int argc, const char * *argv)
{
  if( argc <= 1 || ipExistsArgument(argv, "-usage") || ipExistsArgument(argv, "-help") )
    ...
    cout
    <<
    "usage: ImageStat infile [-outbase outbase] [-extractLabel <label>] [-combine infile2 [-relabel]] [-type byte|short] [-threshold tmin,tmax] [-mask infile2] [-constOper opID,val] [-extension ext] [-v]"
    << endl;
    cout << endl;
    ...
    cout
    << "-constOper opID,val       apply the following operation to the image: I op val, op = +/0, -/1, */2, //3"
    << endl;
    cout << "-v                   verbose mode " << endl;
    cout
    <<
    "-type byte|short    Type of processing image (computations are always done with unsigned char images), default is short"
    << endl;
  ...
  if( combineFile )
    ...
    if( debug )
      {
      cout << "thresholding image  " << (int) tmin << "," << (int) tmax << endl;
      }
