...
int main(int argc, const char* argv[])
{
  ...
  if( argc <= 1 || ipExistsArgument(argv, "-h ") || ipExistsArgument(argv, "-help") ||
    ...
    cout << "usage: " << argv[0] << endl
         ...
         << "              and depth(d)" << endl
         <<
    " -fEmbed <textfilename> same use as the embed option, but px,py,pz,w,h,d are written in the textfile, in that order"
         << endl
  ...
  if( zpacksOn )
    ...
    if( debug )
      {
      cout << "size of the original image " << fullRegion.GetSize(0)
    ...
    if( debug )
      {
      cout << "cropping zpacks: " << zpackNumber << "*" << NumSlices <<   endl;
      }
    ...
    if( debug )
      {
      std::cout << "nbslice: " << nbslices << " nbinter: " << nbinter << std::endl;
      }
    ...
    if( remainer != 0 )
      {
      std::cerr << "The number of slice (" << nbslices << ") along this axis is not a multiple of " << nbinter
                << std::endl;
      }
    else
      {
      ...
      if( nbslices != coilid_list.size() )
        {
        std::cerr << "Coil ID list has a different number of line than the number of slices (" << coilid_list.size()
                  << " Vs. " << nbslices << ")" << std::endl;
        return -1;
        }
      ...
      if( debug )
        {
        std::cout << "Output images, initialized" << std::endl;
        }
      ...
      // If the extension contains a gz, then we get rid of it to get the real extension, and add it when creating the
      // file name
      if( extension == "gz" )
        {
    ...
    if( regionOn )
      ...
      if( maxx < imageRegion.GetSize(0) - autoBorder )
        {
        maxx += autoBorder;
        }
      else
        {
        maxx = imageRegion.GetSize(0) - 1;
        }
      if( maxy < imageRegion.GetSize(1) - autoBorder )
        {
        maxy += autoBorder;
        }
      else
        {
        maxy = imageRegion.GetSize(1) - 1;
        }
      if( maxz < imageRegion.GetSize(2) - autoBorder )
        {
        maxz += autoBorder;
        }
      else
        {
        maxz = imageRegion.GetSize(2) - 1;
        }
    ...
    if( debug )
      {
      cout << "size of the original image " << dim[0]
           << "," << dim[1] << "," << dim[2] << endl;
      }
    if( debug )
      {
