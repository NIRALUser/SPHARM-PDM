...
int main(const int argc, const char * *argv)
{
  if( argc <= 1 || ipExistsArgument(argv, "-usage") || ipExistsArgument(argv, "-help") )
    ...
    cout << "-outfile outfile       outfilename, will only be applied to main output (if multiple output are given)"
         << endl;
    ...
    cout << "-combine infile2       combine the inputfile by interpreting them as labelfiles. " << endl
         <<
    "   -relabel              labels in infile2 will be relabeled to succeed labels in infile (no label overlap)"
         << endl;
    ...
    cout
    << "-threshMask min,max    mask threshold: everything I < min , I > max is set to 0, otherwise is left as is"
    << endl;
    cout << "-mask infile2          use infile2 as a binary mask (intensities > 0 ) and combine with inputfile  "
         << endl;
    cout << "-constOper opID,val    apply the following operation to the image: I op val, op = +/0, -/1, */2, //3"
         << endl;
    ...
    cout
    << "-matchHistoPara bins,points,thresh  optional parameters for matchHistogram (bins= number of histogram bins"
    << endl;
    cout
    << "                        points = number of control points, thresh = boolean for mean intensity threshol [0/1])"
    << endl;
    cout
    <<
    "-smooth -gauss -curveEvol -grayOpen -grayClose -grayDilate -grayErode -grayFillHole -meanFilter -anisoDiff [-size val] [-iter num]"
    << endl;
    cout << "                       smoothing of image using any of the mentioned smoothing filters" << endl;
    cout
    << "                       size is stuctural element size, variance, or timestep depending on the filter choice"
    << endl;
    cout << "                       iter is number of iterations" << endl;
    cout << "-v                     verbose mode " << endl;
    cout
    <<
    "-type byte|short|float Type of processing image (computations are always done with float images), default is short"
    << endl;
    cout << "-extension ext         Extension of the output (determines output format)" << endl;
    cout
    <<
    "-conComp Lbl           For a binary image, rank all the connected components according to their sizes and create a binary image with the 'Lbl' biggest ones"
    << endl;
    cout
    <<
    "                       if Lbl=0 outputs the labeled image with all the components labeled according to their size"
    << endl;
    cout << "-changeOrig px,py,pz   Change the orgine of the image" << endl;
    cout
    <<
    "-createIm X,Y,Z,px,py,pz Create an empty image with the specified parameters: image dimensions X,Y,Z, image resolution px,py,pz"
    << endl;
    cout
    << " -crop px,py,pz,w,h,d   cropimage: origin px,py,pz (startindex is 0) dimensions width(w),height(h), depth(d)"
    << endl;
    cout << "-changeSp spx,spy,spz  Change the spacing of the image" << endl;
    cout << "-max infile2           Compute the maximum between the corresponding pixels of the two input images"
         << endl;
    cout << "-min infile2           Compute the minimum between the corresponding pixels of the two input images"
         << endl;
    cout << "-avg infile2 infile3...       Compute the average image" << endl;
    cout
    <<
    "-majorityVoting infile2 infile3...       Compute an accurate parcellation map considering a majority voting process"
    << endl;
    ...
    cout
    <<
    "-std infile2 infile3...             Compute the standard deviation from a set of images (one image has to be specified for the input, but it's not included in the process)"
    << endl;
    cout << endl << endl;
    exit(0);
    }
  ...
  if( typeChat && !strcmp(typeChat, "byte") )
    {
    writeByte = true;
    }
  if( typeChat && !strcmp(typeChat, "float") )
    {
    writeFloat = true;
    }
  ...
  if( tmp_str )
    ...
    if( 6 != num )
      ...
      std::cout << "Val: " << Dims[0] << " | " << Dims[1] << " | " << Dims[2] << " | " << Dims[3] << " | "
                << Dims[4] << " | " << Dims[5] << std::endl;
      }
    }
  ...
  if( pixelType == itk::ImageIOBase::SYMMETRICSECONDRANKTENSOR
    ...
    if( !changeSpOn && !changeOrigOn && !editPixdimsOn && !cropOn && !center  && !maskOn )
      ...
      std::cerr
      <<
      "The only operations supported on Diffusion Tensor Images are: -editPixdims, -changeSp, -changeOrig, -crop , -mask and -center"
      << std::endl;
      return EXIT_FAILURE;
      }
...
//  if (debug) cout << "Loading file " << inputFileName << endl;
  ...
  if( combineFile )
    ...
    if( debug )
      {
      cout << "Loading file2 " << combineFile << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "extracting object " << extractLabel << endl;
      }
    ...
    if( debug )
      {
      cout << "thresholding image  " << tmin << "," << tmax << endl;
      }
    ...
    if( debug )
      {
      cout << "threshold/mask image  " << tmaskmin << "," << tmaskmax << endl;
      }
    ...
    if( debug )
      {
      cout << "Loading file2 " << maskFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Loading file2 " << addFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Loading file2 " << subFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Loading file2 " << mulFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Loading file2 " << divFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Loading file2 " << matchHistogramFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      cout << "Operation  " << operID << "," << operVal << endl;
      }
    ...
    if( debug )
      {
      cout << "Get all the  " << Lbl  << " bigger elements " << endl;
      }
    ...
    if( debug )
      {
      cout << "dilate ball radius  " << dilateRadius  << ", val " << dilateVal << endl;
      }
...
//    inputImage->SetSpacing(spacingval);
    ...
    if( debug )
      {
      cout << "dilate ball radius  " << erodeRadius << ", val " << erodeVal << endl;
      }
    StructuringElementType structuringElement;
    ...
    if( debug )
      {
      cout << "size of the original image " << dim[0] << "," << dim[1] << "," << dim[2] << endl;
      }
    if( debug )
      {
      cout << "cropping (x,y,z,w,h,d) " << extractRegion.GetIndex(0) << ","
    ...
    if( curveEvolOn )
      ...
      cout << "smoothing  (gradient anisotropic Diffusion ): size  " << smoothSize << ", iterations " << numIter
           << endl;
    ...
    ImagePointer csfImage, wmImage, gmImage, restImage;
      ...
      if( debug )
        {
        cout << "Loading image " << csfFile  << endl;
        }
      VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
      ...
      if( debug )
        {
        cout << "Loading image " << wmFile  << endl;
        }
      VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
      ...
      if( debug )
        {
        cout << "Loading image " << gmFile  << endl;
        }
      VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
...
//    inputImage->SetSpacing(pixdims);
    ...
    if( debug )
      {
      cout << "Loading file2 " << MaxFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    imageReader->SetFileName(MaxFile);
    if( debug )
      {
      cout << "computing maximum  " << endl;
      }
    ...
    if( debug )
      {
      cout << "Loading file2 " << MinFile  << endl;
      }
    VolumeReaderType::Pointer imageReader = VolumeReaderType::New();
    ...
    if( debug )
      {
      std::cout << "Applying power: " << pwrval << std::endl;
      }
    ...
    if( debug )
      {
      cout << "Computing average image " << endl;
      }
    ...
    if( debug )
      {
      cout << "Computing standard deviation image" << endl;
      }
    ...
    for( int LabelFileNumber = 0; LabelFileNumber < NbFiles; LabelFileNumber++ )
      ...
      if( debug )
        {
        cout << "Loading file " << InputFiles[LabelFileNumber] << endl;
        }
    ...
    Radius.Fill(1);
    NeighborhoodIteratorType NeighborhoodOutputIterator(Radius, MajVotingImage,
                                                        MajVotingImage->GetRequestedRegion() );
    ...
    for( OutputIterator.GoToBegin(), NeighborhoodOutputIterator.GoToBegin(); !OutputIterator.IsAtEnd();
         ++OutputIterator, ++NeighborhoodOutputIterator )
      {
      PixelType  MaxVoxelValue = 0, MaxLabelValue = 0;
      PixelType *LabelArray;
      LabelArray = new PixelType[MaxLabel];
  ...
  if( debug )
    {
    cout << "writing output data " << outFileName << endl;
    }
