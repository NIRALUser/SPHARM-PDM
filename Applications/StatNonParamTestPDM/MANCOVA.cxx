...
void doMANCOVATesting( unsigned int numSubjects, unsigned int numFeatures, unsigned int numGroupTypes, unsigned int numIndependent, unsigned int testColumn, unsigned int numPerms, vnl_matrix<int> * groupLabel, vnl_matrix<double>  * &featureValue, bool interactionTest, unsigned int numA, unsigned int numB, double significanceLevel, int computeStatisticsType, vnl_vector<double>& rawP, PointsContainerPointer &meanPoints )
{
  ...
  for ( unsigned int feat=0; feat<numFeatures; feat++ )
    ...
    origStatistics(feat) = computeStatisticValue( A, B, C, X, Y, E, H, precompAA, precompXPseudoInverse, NDtmp, PPtmp, DDtmp, NNtmp, AAtmp, Ctmp, computeStatisticsType );
  ...
  for ( unsigned int nP=0; nP<numPerms; nP++ )
	...
	Y.set_column(tup, featureValue->get_column(feat*tupelSize+tup) ); 
      ...
      currentStatistic = computeStatisticValue( A, B, C, XP, Y, E, H, precompAA, precompXPseudoInverse, NDtmp, PPtmp, DDtmp, NNtmp, AAtmp, Ctmp, computeStatisticsType );
