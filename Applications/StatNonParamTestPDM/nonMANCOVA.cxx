    ...
    for( unsigned int feat = 0; feat < numFeatures; ++feat )
      ...
      for( unsigned int sub = 0; sub < numSubjects; ++sub )
        ...
        for( unsigned int tup = 0; tup < tupelSize; ++tup )
          ...
          dists(sub) +=
            ( (*featureValue)[sub][feat * tupelSize
                                   + tup]
              - currentMeanPoint[tup]) * ( (*featureValue)[sub][feat * tupelSize + tup] - currentMeanPoint[tup]);
          }
      ...
      spearmanRhoPro(feat) = computePearsonCorrelationWithP( dataRanked, projsRanked, dPSP_diff, dPSP_positive,
                                                             dPSP_negative );
      spearmanRhoDist(feat) = computePearsonCorrelationWithP( dataRanked, distsRanked, dPSD_diff, dPSD_positive,
                                                              dPSD_negative );
