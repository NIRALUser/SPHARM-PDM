 ...
 //delete (VTKITKConverter2);
  ...
  try
	...
	regParaTemplateMesh->Update();
        ...
        if (debug) std::cout << "Calculating parameter rotation" << std::endl;
        ...
        for(int i=0; i<3; i++)
        {
          ...
          if(phiPts[i] < 0) phiPts[i] = phiPts[i] + 2 * M_PI; //phiPts: 0...2*M_PI
        }
        ...
        */       
        ...
        for(int i=0; i<3; i++)
        {
          if(rotPts[i][2] > 1) rotPts[i][2] = 1;
          if(rotPts[i][2] < -1) rotPts[i][2] = -1;
          rotTheta[i] = acos(rotPts[i][2]); // 0 .. M_PI
          rotPhi[i] = atan2(rotPts[i][1],rotPts[i][0]);
          if(rotPhi[i] < 0) rotPhi[i] = rotPhi[i] + 2 * M_PI;
        }

        int phiFlag = 0;
        double testPhi3 = rotPhi[2] - rotPhi[1];
        if (testPhi3 < 0) testPhi3 = testPhi3 + 2 * M_PI;
        if (testPhi3 > M_PI) phiFlag = 1;
        ...
        if(debug) std::cout << "Final param.s of template points: " << std::endl;
        for(int i=0; i<3; i++)
        {
          testTheta[i] = acos(testPts[i][2]) ; // 0 .. M_PI
          testPhi[i] = atan2(testPts[i][1],testPts[i][0]);
          if(testPhi[i] < 0) testPhi[i] = testPhi[i] + 2 * M_PI;
          if(debug) std::cout << "ThetaPhi[" << i << "] = (" << testTheta[i] << "," << testPhi[i] << "); ";
        }
...
std::cout<<"procalign"<<std::endl;
      ...
      if (writePara) {
        if (debug) std::cout << "writing para mesh data" << std::endl;
	...
	{ outFileName.erase();
	  ...
	  for (int i = 0; i < nvert; i++) {
	    ...
	    if (phi > M_PI) phi = 2 * M_PI - phi; // 0 .. M_PI ..0
	    efile << phi<<endl;
	    //if (i != nvert - 1) efile << " ";
	  }
	  efile.close();
