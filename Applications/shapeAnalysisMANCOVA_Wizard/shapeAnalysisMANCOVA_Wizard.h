...
//********************make a new file****************************************************************
	...
	void openSelectionFolderDialog();//select the folder where the file will be save (path to the folder save in the QString path)

//******************working on the Qtablewidget**********************************************************
	void modifyHeader();//to know we want to change a header(pushbutton) :modifyHeaderName[0]=1, otherwise=0  (size of modifyHeaderName=1)
	void openColumnTitleDialog(int);//if modifyHeaderName[0]=1 and if we clik on a header => QInputDialog is openning and we can write the new name of the header
	...
	void selection_Group_IndeVariabl_DataDialog(int c, int r);//if checkbox is checked, we can select infile/group/inde_var columns or unselect one OR if no checkbox is checked and if you click on a box of the infile column => QFileDialog is openning and we can select the data
	void saveInfile();//save the QtableWidget as a csv file (as the same place if we have loaded one or in the QString path with the QString name)
...
//******************ON THE 2ND TAB*************************************************
 ...
 signals:
	...
	std::vector< int > headerVector;//to know the type of the column : if headerVector[i]=0 : i as no type ; if=1 i is the infilecolumn ;if=2 i is a scale column; if= 3 i is a independent variable column ; if=4 i is a group column
	...
	void lineEditInfileOrScaleDisplay(int c, int type);// number of the infile column in the QlineEdit and  paint the foreground of the header (idem for scale)
	...
	void displayLineEditIndependent(int c);//diplay the list of the independent variables columns (list is sort and there is , between the numbers)
