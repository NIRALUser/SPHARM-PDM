...
int CreateMRMLSceneHelper::Write()
{
  ...
  if( !m_InputSceneName.empty() )
    ...
    if( CheckDoublonsWithScene() )
      ...
      std::cerr
      <<
      "Error: Some node names appear inserted are already present in the given scene. Please verify your input information"
      << std::endl;
  ...
  for( unsigned int i = 0; i < m_Arguments.size(); i++ )
    ...
    if( !m_Arguments[i]->GetType().compare( "Transform" ) )
      ...
      {
      std::cerr << "An error occured while writing the scene. Unexpected node found. This should never happen"
                << std::endl;
      return 1;
      }
    }
